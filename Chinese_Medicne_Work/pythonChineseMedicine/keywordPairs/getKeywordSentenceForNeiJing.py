...
#初始化，打开相关app 
    ...
    def removeCache(self):
        fp = "C:/Users/Administrator/AppData/Local/Temp/gen_py"
...
if __name__=="__main__":
    ...
    resFileName = '内经.xlsx'    #input("请输入源文件名：")
    xlsPath = os.path.join(pathFile, resFileName)
    finalDir = os.path.join(pathFile, '主题词-' + resFileName.replace('.xlsx', ''))
    ...
    for keyword in keywords:
        ...
        xlsSt.AutoFilter(Field=1, Criteria1='*in_sentence*'+keywordCuts[1]+'*', Operator=1,
                         Criteria2='*')
        xlsSt.Select()
        ...
        if doc.Content.Paragraphs.Count == 1:
            with open('error.txt', 'at') as f:
                f.write(keyword+'\n')
                f.close()
            errorName =  keyword + "-主题词-无-" + resFileName.replace('xlsx', 'docx')
        ...
        doc.Content.Find.Execute(FindText='chapter', ReplaceWith='ABCD', Replace=2)
        doc.Content.Find.Execute(FindText='in_sentence', ReplaceWith='ABC^p句子：', Replace=2)

        # 设置书名样式
        doc.Content.Find.ClearFormatting()
        doc.Content.Find.Replacement.ClearFormatting()
        app.WordApp.Selection.WholeStory()
        while app.WordApp.Selection.Find.Execute('ABCD*ABC', False, False, True,):
            app.WordApp.Selection.Font.Bold = True
        ...
        app.WordApp.Selection.WholeStory()
        cnt = 1
        while app.WordApp.Selection.Find.Execute('ABCD', False, False, True, False, False, True, 1, False, '^p（'+str(cnt)+'）', 1,):
        ...
        doc.Content.Find.Execute(FindText='ABC', ReplaceWith="^p关键词："+keywordCuts[1], Replace=2)

        doc.Content.Find.ClearFormatting()
        doc.Content.Find.Replacement.ClearFormatting()
        app.WordApp.Selection.WholeStory()
        while app.WordApp.Selection.Find.Execute(keywordCuts[1], ):
            app.WordApp.Selection.Font.Bold = False
            app.WordApp.Selection.Font.Name = "宋体"
            app.WordApp.Selection.Font.Color = 0
        ...
        outputName = keyword + "-主题词-" + resFileName.replace('xlsx', 'docx')
        outputPath = os.path.join(finalDir, outputName)

        doc.SaveAs2(outputPath)

        doc.Close()
        print(keyword+"已经处理完毕")
    xls.Close(0)
